...
def main():
    ...
    for i, job in enumerate(jobs):
        ...
        # Cost tier: reminder-class jobs run on Sonnet (~5x cheaper on
        # output/cacheWrite, which dominates cron cost); analysis jobs that
        # reason over user data keep the resolved (Opus) model. See
        # create-reminder.sh for the same split.
        _nl = name.lower()
