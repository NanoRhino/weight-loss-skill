...
**Single-meal volume check.** For every meal in the 7-day plan, verify the total food volume is realistic for one sitting. A meal may fit the calorie target perfectly but still be too much food to physically eat at once — especially when combining a full grain portion, multiple vegetable sides, protein, and extras. If a meal feels too large, move overflow items (extra fruit, yogurt, secondary sides) into a snack slot and add a `💡` tip: "如果一餐吃不完，可以把 [items] 留到加餐，时间自由安排" / "If this is too much for one meal, save [items] as a snack — eat whenever works for you." This applies to all meals including breakfast (many users have smaller morning appetites).
