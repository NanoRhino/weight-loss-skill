# Chat Message Formatting

- Plain text with emoji section markers — no Markdown tables (most chat apps don't render them)
- Use emoji to separate sections: 📝 food log, 📊 progress, ⚡ right now suggestion, 💡 next time suggestion, ✨ nice work
- Round values to whole numbers (calories) or one decimal (macro grams)
- Prefix estimated food portions with `~`
- Tone: friendly, concise, encouraging — no lecturing
- Always reply in the user's language
- Standardized foods (a can of Coke, one egg) → record directly, no portion follow-up needed
