...
| Result | Action |
|--------|--------|
| `level_up == true` AND `badge_image` exists | Append badge celebration + send image via `MEDIA: {badge_image}` (badge_image is a local absolute path generated by badge-calc.py) |
