...
- `alert: true` + `!blocker_talked` → ③ **REPLACED by blocker discussion** (do NOT give a normal suggestion separately):
...
- ③ **REPLACED by follow-up**: "之前的方案不太好执行吗？" Offer adjustment with new angle. Second refusal → dismiss.
