...
- **ONE soft bridge line allowed — a reminder opt-in (non-gating).** After the celebration and the ①②③ breakdown, you MAY add **exactly one** short, low-friction line that surfaces the meal-reminder opt-in — e.g. "I'll nudge you around breakfast/lunch/dinner so logging stays easy — just tell me if you'd rather different times." This is the deliberate hook that converts one good evening into a day-2 return. The default reminders (08:30 / 12:30 / 18:30 local, reminder at meal−15) plus the weekly weight check-in are created via `notification-manager`'s `batch-create-reminders.sh --only meal,weight --skip-existing`; this line is the user-facing surfacing of the meal reminders, not a question that must be answered before anything else happens.
