...
# Leave is tracked SOLELY by the local data/leave.json (start/end/reason). The
# former DB silence sync (POST /v1/lifecycle/silence to 127.0.0.1:3100) is gone —
# the lifecycle API was never deployed. pre-send-check.py reads leave.json
# directly to suppress reminders during leave; no cross-process state to sync.
...
def cmd_set(args):
    ...
    # leave.json IS the source of truth — pre-send-check.py reads it directly to
    # suppress reminders during the leave window. No DB sync needed.
...
def cmd_clear(args):
    ...
    if existed:
        try:
            ...
            pass
    # Deleting leave.json IS the un-leave — pre-send-check.py stops suppressing
    # the moment the file is gone. No DB to un-freeze.
