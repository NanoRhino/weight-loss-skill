...
def check_habit_checkin(workspace_dir, meal_type, tz_offset):
    """Check habits.active for any habit due for check-in at this meal.

    Returns (checkin_text, habit_id) or (None, None) if no habit is due.
    """
    ...
    try:
        ...
        log(f"Warning: could not read habits.active: {e}")
        return None, None
    ...
    # Meal type normalization for matching
    meal_aliases = {
        "breakfast": ["breakfast", "meal_1"],
        "meal_1": ["breakfast", "meal_1"],
        "lunch": ["lunch", "meal_2"],
        "meal_2": ["lunch", "meal_2"],
        "dinner": ["dinner"],
    }
    match_meals = meal_aliases.get(meal_type, [meal_type])

    for habit in habits:
        habit_id = habit.get("habit_id", "")
        check_in_meal = habit.get("bound_to_meal") or habit.get("check_in_meal")

        # If no explicit meal binding, use type heuristic
        if not check_in_meal:
            h_type = habit.get("type", "")
            trigger = (habit.get("trigger", "") or "").lower()
            desc = (habit.get("description", "") or "").lower()

            # Morning habits → breakfast
            if any(kw in trigger or kw in desc for kw in ["起床", "早上", "晨", "morning", "waking"]):
                check_in_meal = "breakfast"
            # Lunch habits → lunch
            elif any(kw in trigger or kw in desc for kw in ["午", "lunch", "中午"]):
                check_in_meal = "lunch"
            # Dinner habits → dinner
            elif any(kw in trigger or kw in desc for kw in ["晚", "dinner", "evening"]):
                check_in_meal = "dinner"
            else:
                # Default: first meal of the day
                check_in_meal = "breakfast"

        if check_in_meal not in match_meals:
            continue

        # Check if already mentioned today
        mention_log = habit.get("mention_log", [])
        already_mentioned = any(
            (m.get("date", "") if isinstance(m, dict) else str(m)) == local_date
            for m in mention_log
        )
        if already_mentioned:
            log(f"Habit {habit_id} already mentioned today")
            continue

        # Build check-in text based on habit description
        tiny = habit.get("tiny_version", "") or habit.get("description", "")
        trigger = habit.get("trigger", "")

        # Generate a natural check-in question
        checkin_templates_zh = [
            f"对了，{trigger}有{tiny}吗？",
            f"今天{tiny}了吗？",
            f"顺便问一句，{tiny}了没？",
        ]
        checkin_templates_en = [
            f"By the way — did you {tiny} today?",
            f"Quick check: {tiny}?",
        ]

        # Pick template based on description language
        import hashlib
        day_hash = int(hashlib.md5(local_date.encode()).hexdigest(), 16)
        if any('\u4e00' <= c <= '\u9fff' for c in tiny):
            checkin = checkin_templates_zh[day_hash % len(checkin_templates_zh)]
        else:
            checkin = checkin_templates_en[day_hash % len(checkin_templates_en)]

        return checkin, habit_id

    return None, None


def update_habit_mention_log(workspace_dir, habit_id, tz_offset):
    """Record that a habit was mentioned today."""
    habits_path = os.path.join(workspace_dir, "habits.active")
    if not os.path.exists(habits_path):
        return
    ...
    for habit in habits:
        if habit.get("habit_id") == habit_id:
            if "mention_log" not in habit:
                habit["mention_log"] = []
            habit["mention_log"].append({"date": local_date, "source": "pre-send-check"})
            break
