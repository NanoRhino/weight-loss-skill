...
| 最近一次是文字打卡（无照片） | 提醒拍照+参照物更准（"拍张照发我，旁边放双筷子/勺子我估量更准"） |
| 拍照但无参照物，`hint_count == 0` | 轻提："下次拍的时候旁边放双筷子或勺子，我估量能更准哦" |
| 拍照但无参照物，`hint_count == 1` | 加重："放个参照物（筷子、勺子、或握个拳头入镜）我能估得更准，试试看？" |
| `hint_count >= 2` | 不再提醒，尊重用户选择 |

提醒后 `hint_count += 1` 并更新 `last_hint_date`，写回文件。参照物根据用户饮食习惯推荐（中餐→筷子/勺子，西餐→叉子/刀，默认→筷子/勺子），拳头（~一个成年人拳头≈一份米饭/水果的量）通用。
