...
> **Cold-Start v3（调度模型）：** cron 不再是带标签的一次性任务。openclaw-infra 现在只挂**一条周期性的"sweep"cron**（约每 2h 跑一次），payload 是**通用指令**（"跑 activation 前置检查，gate 放行就发当前该发的那一条"）——它**不再告诉你发第几条**。该发哪一条由 `pre-send-check.py` 计算：`index = nudges_sent + 1`，且只在 `now - claimedAt ≥ 该触点阈值`（touch1=4h / touch2=24h）**且** `now - last_nudge_at ≥ ~20h`（MIN_GAP，防 sweep 追赶时挤作一堆）时放行。**冷启动注册（COLD，无 PLAN.md）的用户也会进这个流程**（v1/v2 时他们不被调度）。

> **"冷"是行为定义，不是"无方案"：** 无打卡 + 零入站 = 冷，与有没有 PLAN.md 无关。此 cohort 约 86% 走 TDEE handoff 进来、**有完整 PLAN.md**（"拿了方案就沉默"），约 14% 才真正无方案。所以下面默认面向"有方案的沉默用户"，COLD 只是少数分支。

> **序列已从 4 条缩到 2 条（Track A，2026-06-24）：** WARM 召回数据显示触点 3、4（T+3d/T+7d）零召回，只保留 T+4h、T+24h；cap=2。

**pre-send-check 已在第三步用 `--meal-type activation` 把过这道关**——决定性判据是 `channel-source.json > lastInboundAt` 存在即取消（用户已回复）；另外 `ACTIVATION_ENABLED` 关停开关（env 设为 0/false → 全部 NO_REPLY）/ 未成年（结构化 age<18）/ 已完成引导 / 记过餐 / 请假 / 沉默 / 上限（nudges_sent≥2）/ 当前触点未到阈值 / 被 MIN_GAP 压住 → 都 NO_REPLY。本流程只在返回 `SEND activation ...` 时执行。
...
1. **从第三步 `pre-send-check.py` 的输出行取 `nudgeIndex` 和 `nudgeAngle`。** 输出形如 `SEND activation nudgeIndex=2 nudgeAngle=photo`。`nudgeIndex`（整数 1–2）是选哪条文案的**唯一依据**；`nudgeAngle`（`value_first` | `photo`）是该 index 的语义角度（与下表一致）。**旧的 `(nudge=N)` 和 `(nudgeIndex=N, nudgeAngle=X)` payload token 都已移除——不要再在 cron 指令里找它们。** 若输出不是 `SEND activation ...`（例如 `NO_REPLY`），回 `NO_REPLY`，不发。
2. 读 `USER.md` 的 `Name` 和 `Language`；**冷暖分流：检查 workspace 根有没有 `PLAN.md`**——有 → WARM（多数），可从 `PLAN.md` 取**真实每日目标热量+蛋白质**做个性化、并提"你前阵子设好的方案还存着"；没有 → COLD（少数），**绝不报具体数字、绝不提"方案/计划/资料/都准备好了"**，走对应的 COLD 文案。
3. 按 `nudgeIndex` 选对应文案（递减门槛）。两条角度、内容、以及每条的 WARM/COLD 写法详见 `references/recall-messages.md` § 激活提醒：

   | index | angle | 门槛 | 要点 | ask |
   |-------|-------|------|------|-----|
   | 1 | `value_first` | 中 | （WARM）"你前阵子在我们这儿做了方案、一直没真正开始，热量+蛋白质目标都还存着、随时能用，要不要试一下？拍下下一餐我来算宏量"（生产验证过的回流文案）；（COLD）不提方案/数字，低门槛 demo 邀请 | 拍/报下一餐（一个低门槛动作） |
   | 2 | `photo` | 低 | "随口报一餐我来算，免 app、免记账（也可以拍照）"（标签 `photo` 是历史命名，文案现以文字报餐为主） | 一句话报餐（也可拍照） |

   index 2（photo）是**最后一条**；它之后不再有触点（cap=2 + 此条是序列终点）。
4. 把选定文案**本地化**到 `USER.md > Language`，WARM 时可用 `PLAN.md` 的真实数字（目标热量/蛋白质）个性化（只有真有数字才报）。保持 SMS 简短，NanoRhino 用驼峰写法，**不要 🦏 emoji**。
...
- 互动阶段（stage）：由 `{notification-manager:baseDir}/scripts/lifecycle-check.py` 本地确定性推导（2/4 阶梯，按 days_silent + 本地 recall 计数；3100 DB API 从未部署，stage 不落库）。读取方式 = `pre-send-check.py` 输出的 `stage=N`（其内部已调用 resolver）。`check-stage.py` 已删除。
