...
- **`SEND leave_ending=true end=YYYY-MM-DD`** → 用户的请假明天结束。在当天最后一条提醒里加一句"明天开始恢复打卡～准备好了随时来"。
...
> ⚠️ **假期提醒（仅 Stage 1 早餐）：** 如果最近 5 天内有中国法定节假日，且用户没有活跃的请假记录（`leave.json` 不存在或 `active: false`），在开场白里顺便问用户假期有没有出去玩的计划、哪几天不方便打卡。用户回复了具体日期后，调用：
> ```bash
...
> ```
> 用户说"不需要"或"在家"→ 不创建 leave，正常提醒。用户说"还没定"→ 明天再问一次。
>
> **假期期间用户主动打卡** → 正常记录，不拦。leave 只影响主动提醒，不影响用户主动发消息。
>
> **用户提前说"我回来了"** → 调用：
> ```bash
> python3 {baseDir}/scripts/leave-manager.py clear --data-dir {workspaceDir}/data --tz-offset <tz>
> ```
>
> **查询请假状态：**
> ```bash
> python3 {baseDir}/scripts/leave-manager.py info --data-dir {workspaceDir}/data --tz-offset <tz>
> ```
