...
1. 解析 cron payload 中的 `nudge=1`（约 T+4h，当天白天）或 `nudge=2`（T+24h，最后一条），决定语气。
2. 读 `USER.md` 的 `Name` 和 `Language`；读 `data/engagement.json > recall_topics` 去重。
3. **检测冷暖路径（决定用哪套文案）：检查 workspace 根目录是否存在 `PLAN.md`。`PLAN.md` 存在 → WARM 路径（用户走过 TDEE handoff，方案已就位）→ 用 handoff 风格文案（"方案都准备好了…随口报一餐就行"）。`PLAN.md` 不存在 → COLD 路径（冷启动用户，无方案、无完整资料）→ 用通用零门槛 "发我上一餐" 钩子，绝不提及方案/计划/资料。** 两套文案均见 `references/recall-messages.md` § 激活提醒。
4. 按 `references/recall-messages.md` § 激活提醒 中对应路径（WARM 或 COLD）的策略生成消息。**本地化**到 `USER.md > Language`，给零门槛的例子，nudge 2 点名暂停出口（"pause"）。
5. **直接输出消息文本**（announce 投递，见第五步）。
6. **发送后，确定性地 +1 计数**——**不要手写 engagement.json**。调用脚本：
   ```bash
   python3 {notification-manager:baseDir}/scripts/activation-mark-sent.py \
     --workspace-dir {workspaceDir} --counter nudges_sent
   ```
7. 同时追加一条 `recall_topics`（≤10 字，去重用，read-modify-write）。
8. **只在确实输出了消息后才调 mark-sent**；判断不该发则回 `NO_REPLY` 且不调脚本。
