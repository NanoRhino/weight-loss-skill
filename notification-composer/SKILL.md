...
> ✅ **必须用 message 工具发送：** Cron 系统不会自动投递你的回复。compose 完最终提醒文本后，调用 `message` 工具（`action: "send"`）发送给用户。发送成功后回复 `NO_REPLY`（避免重复）。如果判断不需要发送，直接回复 `NO_REPLY`。
>
> 不要把推理过程、检查结果或叙述放进发送内容——只发最终的提醒文本。
...
compose 完成后，用 `message` 工具发送给用户：

- `action`: `"send"`
- `message`: 最终提醒文本（不含任何推理、检查结果或叙述）

不指定 `target`——自动路由到当前用户的聊天频道。
