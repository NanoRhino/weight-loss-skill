...
1. Run `pre-send-check.py --meal-type guided-feedback --question-id <id>`
   - `NO_REPLY` → stop
   - `SEND` → pre-send-check automatically marks status as `asked` and writes short-term hint
2. Read template from `references/guided-feedback-templates.md`
3. Output the question message (adapt wording to user's locale/style)

### Reply Processing (Handling replies)
...
2. Map answer to preference value using `references/guided-feedback-templates.md`
