...
Templates: `references/guided-feedback-templates.md`
State script: `{notification-manager:baseDir}/scripts/guided-feedback-state.py`

### Trigger

Cron message: `"Run notification-composer for guided-feedback <question-id>."`

### Flow

1. Run `pre-send-check.py --meal-type guided-feedback --question-id <id>`
   - `NO_REPLY` → stop
2. Read template from `references/guided-feedback-templates.md`
3. Run state script: `update --question-id <id> --new-status asked`
4. Output the question message

### Reply Processing

1. If a question has `status: "asked"`, check if reply is an answer (number, phrase, or free-text) vs unrelated content (food/exercise → route normally)
2. Run state script: `update --question-id <id> --new-status answered --answer <text>`
   - Response includes `chain_next` — the next question to ask in the chain
3. Apply answer → preference mapping (see `references/guided-feedback-templates.md`)
4. Update `ai-preferences.md` (or `health-preferences.md` for food-preference)
5. Confirm to user — one brief sentence, separate message
6. If `chain_next` is not null → send that question immediately as a separate follow-up
7. If `chain_next` is null → trigger `notification-manager` for next-day scheduling
