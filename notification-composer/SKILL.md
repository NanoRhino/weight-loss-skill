...
**参照物提醒：** 发拍照邀请前，读 `{workspaceDir}/data/photo-reference-hints.json`（不存在视为 `hint_count: 0`）。如果 `hint_count < 2`，在拍照邀请里自然地加一句参照物提示（筷子、勺子、或握拳入镜），然后 `hint_count += 1` 并写回文件。`hint_count >= 2` 则不再提。参照物根据用户饮食习惯推荐（中餐→筷子/勺子，西餐→叉子/刀，默认→筷子/勺子），拳头（~一个成年人拳头≈一份米饭/水果的量）通用。
