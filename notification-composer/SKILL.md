...
每周一次（周一，第一餐提醒时），运行 `{weekly-report:baseDir}/scripts/weekly-low-cal-check.py`。如果 `below_floor` 为 true，在下一条餐前提醒中加入温和提示。如果 `Health Flags` 包含 `history_of_ed`，完全跳过。用语见 `diet-tracking-analysis` SKILL.md。
