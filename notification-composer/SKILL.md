...
当 cron message 包含 `activation` 时执行此流程。对象是 handoff 进来、收到欢迎消息但**从未回复**的用户。cron 由 openclaw-infra 创建，4 触点序列：step1 T+4h、step2 T+24h、step3 T+3d、step4 T+7d。

**pre-send-check 已在第三步用 `--meal-type activation` 把过这道关**——决定性判据是 `channel-source.json > lastInboundAt` 存在即取消（用户已回复）；另外未成年（结构化 age<18）/ 已完成引导 / 记过餐 / 请假 / 沉默 / 上限（nudges_sent≥4）→ 都 NO_REPLY。本流程只在返回 `SEND` 时执行。

**内容由 `nudgeIndex` 决定（唯一真源），不要用 `nudges_sent` 计数器选文案。**

1. **从 cron payload 文本解析 `nudgeIndex` 和 `nudgeAngle`。** payload 是**自由文本的 agent 指令**（不是 JSON），新字段就写在 composer 指令的括号里，形如：`...Otherwise run notification-composer for activation (nudgeIndex=3, nudgeAngle=rapport).` 从这段文本里取 `nudgeIndex=`（整数 1–4）和 `nudgeAngle=`（`value_first` | `photo` | `rapport` | `exit`）。`nudgeIndex` 是选哪条文案的**唯一依据**；`nudgeAngle` 是该 index 的语义角度（冗余校验，应与下表一致）。**旧的 `(nudge=N)` token 已被移除——不要再找它。** 若两个字段都解析不到，回 `NO_REPLY`（payload 异常，保守不发）。
2. 读 `USER.md` 的 `Name` 和 `Language`；读 `PLAN.md` 取**真实的每日目标热量与蛋白质**（个性化用）。
3. 按 `nudgeIndex` 选对应文案（递减门槛）。四条角度与内容详见 `references/recall-messages.md` § 激活提醒：

   | index | angle | 门槛 | 要点 | ask |
   |-------|-------|------|------|-----|
   | 1 | `value_first` | 中 | 复述其每日目标热量+蛋白质，提议"要不要我给你 3 顿简单又能凑够的饭？" | 一个字 YES |
   | 2 | `photo` | 低 | "随手发/拍一餐，我来算，免 app、免记账" | 一张照片 |
...
> 🚫 **本流程严禁手动 Edit/Write `data/engagement.json`，也不要在 activation 路径写 `recall_topics`。** `nudges_sent` 计数**只能**通过 `activation-mark-sent.py --counter nudges_sent` 递增——该脚本已持有 flock 并用原子 `os.replace` 写入。本路径里任何对 engagement.json 的自由编辑都会与该脚本竞争、导致本可成功的 nudge run 报 `error`（见 050208 事故）。activation 触点用 `nudgeIndex` 选内容，**不需要** recall_topics 去重（去重只用于阶段召回，不用于这条固定序列）。
