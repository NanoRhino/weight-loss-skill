...
如果 `days_silent` 为 1（来自第二步），在当天第一个 meal cron 前加一句**温柔提醒（Gentle Nudge）**。见 § 温柔提醒。仅 Day 2（days_silent=1）加 nudge，Day 1 正常不加。
...
**days_silent=2（Day 3）— 情绪+内容召回：**
从 `references/recall-messages.md` 的 S2 Day 3 模板池选取（去重机制见下文）。模板与用户状态相关——可引用上次吃的内容、streak、季节等。

**days_silent=4（Day 5）— 询问是否暂停：**
从 `references/recall-messages.md` 的 S2 Day 5 模板池选取（去重）。给用户选择权，不施压。

**days_silent=3（Day 4）：** pre-send-check 返回 NO_REPLY，不发送。

发送后更新 `recall_history`（见 § 模板去重机制）。
...
**内容类型轮换（不重复）：**
1. 营养知识分享（`s3_nutrition_*`）
2. 功能更新（`s3_feature_*`）
3. 近况询问（`s3_checkin_*`）

从三个类别中**轮换选取**（第1周营养→第2周功能→第3周询问，或按 recall_history 中该阶段已有的类别来排除）。

发送后：
1. 将模板写入 `recall_history`
...
超级开心！从 `references/recall-messages.md` 的回归模板池选取（去重）。然后正常进入餐食对话。不问去哪了，不提间隔。如果对话自然流动，再问要不要恢复提醒。

**回归触发：** 用户发送任何消息（不需要打卡），即重置到 Stage 1。

**完整模板 → `references/recall-messages.md` § 用户回归**

**所有召回/回归消息绝不：** 数错过的天数/餐数 · 鸡汤口号 · 连续打卡语言 · 愧疚式措辞 · 正经的系统通知语气 · 脱离食物/营养的纯抽象关心。

### 模板去重机制

所有召回/nudge/回归消息都通过 `recall_history` 去重，确保同一用户不会收到重复的话术。

**读取：**
```python
recall_history = engagement_data.get("recall_history", [])
used_ids = [r["template_id"] for r in recall_history if r.get("template_id")]
```

**选择模板：**
1. 从当前阶段/类型的模板池获取所有可用模板
2. 排除 `used_ids` 中已有的 template_id
3. 从剩余模板中随机选一个
4. 如果该类别模板全用完 → 清空该类别的 recall_history 记录（仅清空对应前缀，如 `s2_d3_*`），重新选择
...
engagement_data["recall_history"] = recall_history
# write back with read-modify-write pattern
```

**重要：** 用户回到 Stage 1 时**不清空** recall_history。下次再流失时继续排除已用模板。
...
当 Stage = 1 且 `days_silent == 1` 时（仅 Day 2），在当天第一个 meal cron 的消息前加一句温柔提醒。提醒 + 正常内容在同一条消息内。

规则：
- 仅 Day 2（days_silent=1），Day 1 不加。
...
- 从 `references/recall-messages.md` 的 S1 Nudge 模板池选取（去重，写入 recall_history）。

**模板 → `references/recall-messages.md` § S1 温柔提醒**
