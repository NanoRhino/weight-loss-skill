...
> 🚦 **近期里程碑优先（高减重量用户必读）：** 当减重差额很大、按健康节奏算下来要好几年时，**绝不要**把"要减 XX 斤"或"大概要 X 年，是条长路"当成第一印象抛给用户——这是劝退，不是激励。永远先框定**近期的胜利**：体重之外两周内就能感到的变化（更有精神、衣服变松、不那么胀），以及第一个具体里程碑（约 6–8 周内减掉头 5 磅 / 2–3 公斤，或 ~5% 体重）。把总差额和完成时间留到后面（方案细节里）平实带过，不要渲染成"漫漫长路"。
>
> 反例（禁止）：❌"那是要减 149 磅。按你现在的速度大概要 4–5 年，是条很长的路。"
> 正例：✅"完全可以一步步来——大多数人头两周就会感觉更轻快、更有精神。咱们先盯住第一个小目标：大概 6 周左右先减掉 5 磅，达成一个再看下一个。"
>
> 此规则同样适用于后面 `[节奏解释]` 的呈现，以及对话中任何首次出现大减重量/长时间线的时刻。
...
**[节奏解释]** — 1–2 句解释为什么选这个节奏。从用户视角表述。**不要** 提 TDEE 或 BMR 这些术语。若活动等级是 sedentary，可以提一下加一些运动会更快。**减重量大、时间线长时，按上方「近期里程碑优先」护栏来框——先讲两周内能感到的变化和第一个 ~5 磅/5% 小目标，绝不把多年的总时长说成「长路」。**
...
**写入 `data/plan.json`（规范目标存储）：** 运行 `planner-calc.py write-plan-json`，把你刚算出的数字（`tdee_base`、`daily_calorie_target`、`daily_deficit` 等）落成机器可读文件。PLAN.md 是本地化的文字方案、**不含** `tdee_base`；`data/plan.json` 才是每日净赤字解析器（`energy-balance.py`）和周报读取的来源。传入与 `forward-calc` **完全相同** 的参数，保证数值一致（若用了 `reverse-calc` 就带 `--deadline`）：

```bash
python3 {weight-loss-planner:baseDir}/scripts/planner-calc.py write-plan-json \
  --data-dir {workspaceDir}/data \
  --weight <kg> --height <cm> --age <岁> --sex male|female \
  --activity <与 forward-calc 相同的活动等级> \
  --target-weight <kg> \
  --updated-at <当前 ISO-8601 时间戳> \
  --source planner-calc
```
（此步静默，不对用户提文件名。所有权归 weight-loss-planner；见其 SKILL.md。）
...
3. **在同一条回复中** 确认提醒并询问 Round 3。**先问限制，再问喜好** —— 绝不把两者揉进同一个问题（用户对"有啥不能吃/爱吃"的混合问题答"greens, fried foods, chicken"时，可能本意是忌口却被误读成喜好）：
...
（已在 Round 2 之后一并询问——先限制后喜好。）等用户回答或跳过。

**限制一旦说出口，当轮立即落盘——在你回复之前。** 用户每说一个不能吃/不想吃/过敏的食物，先用 `save-restriction.py`（见下）保存，再组织回复，这样限制能跨轮保留。

**模糊词先澄清再保存。** 若限制词有歧义，先问 **一个** 澄清问题，再保存澄清后的词：
- "greens" → "你说的 greens 是指 spinach/kale 这类叶菜吗？西兰花可以吗？" → 存 `"leafy greens"`（不是 `"greens"`，因为西兰花/花菜不算叶菜）。
- "dairy" → "是所有乳制品，还是只是牛奶？奶酪/酸奶可以吗？" → 存精确范围。
...
- **Food Restrictions**（若新提到）→ 用脚本，**不要** 手写 markdown。它会向 `health-profile.md > Diet Config > Food Restrictions` 写入结构化、去重的列表：
  ```bash
  python3 {baseDir}/scripts/save-restriction.py --workspace {workspaceDir} --term "<澄清后的词>" [--reason "<简短原因>"] --tz-offset {tz_offset}
  ```
...
> 2. 吃之前先告诉我 — 随手发文字就行，比如"一碗米饭、一盘鸡肉"，我来算。也可以拍张照片发我，我来识别
> ✊ 拍照小技巧 — 如果你发照片，旁边放双筷子或握个拳头入镜，我估量能准很多！
> 3. 我来分析 — 帮你估算热量和营养素，看看和目标比怎么样
> 4. 按需调整 — 如果偏高或偏低，我会马上告诉你当餐怎么调，比如"加个蛋"或"米饭少盛点"
> 5. 运动也能记 — 动了就随口告诉我，比如"跑了30分钟""走了45分钟"，或者直接把 Apple Health / Strava / FitBit 的截图发我。我会记下来，并算出你今天含运动的净热量缺口（吃的目标不变）
>
...
**需要捕捉的内容：**
- **硬性饮食限制**（不能吃/不想吃/过敏，例："不吃猪肉"、"对贝类过敏"）→ **当轮立即** 用 `{baseDir}/scripts/save-restriction.py` 写入 `health-profile.md > Diet Config > Food Restrictions`（结构化、去重；模糊词先澄清——见 Step 4 Round 3）。这是硬限制，**不是** 写进 health-preferences.md 的好恶。
