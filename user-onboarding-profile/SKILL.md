...
一轮 = 一次模型回复（思考 + 工具调用）。每多一轮，用户就多等 7–15 秒（TTFT + thinking）。每个 turn 的预算：

| Turn 类型 | 预算 |
|---|---|
| **简单路径** —— 收集问答（Round 1–5）、确认、调整方案参数 | **≤ 2 轮** |
| **完整生成** —— 保存画像文件、生成 PLAN.md / MEAL-PLAN.md（含 URL 导出） | **≤ 3 轮** |

守住预算的方法：

1. **Round 1 是一个并行批次，装下所有已知要用的工具。** 所有文件读取（画像文件、schema、plan-export 文档、channel-source.json）、所有计算脚本（`planner-calc.py`、`onboarding-check.py`）、`now.py`，全部放进同一个批次并行调用。绝不把读取拆到多轮。内容已经确定的文件写入/编辑（如用户刚说的偏好）也放进 Round 1——不同文件的写入并行是安全的。
2. **最后一个工具轮和回复轮相邻，中间不插额外轮次。** 写文件不要单独占一轮——能并入已有批次就并入。
3. **依赖刚写文件的脚本 = 同一个 `exec` 命令内用 `&&` 串联。** 同批次的工具调用是并行执行的，顺序无保证。脚本要读本轮正在写的文件时（如 `generate-and-send.sh` 读 PLAN.md/MEAL-PLAN.md），必须在一个 `exec` 里先 heredoc 写文件再跑脚本（见 Step 3 的命令模板），绝不拆成并行兄弟调用或两轮。
4. **不要在 thinking 里起草文件内容或回复。** 思考只做决策（最多几条短要点），内容在工具调用里一次性写出。在 thinking 里把方案/食谱写一遍、再在 write 里重写一遍，生成时间直接翻倍。
5. **工具调用之间不要输出叙述文本**（"现在我来读取..."）。这些文本不会发给用户，纯浪费。
...
在保存步骤开始时 **运行一次** 并复用值——不要重复调用。**且不要单独占一轮：** 把它放进当轮 Round 1 的并行批次里，和文件读取、其他计算脚本一起调用（见「轮次预算」）。
...
- 类似"我已经添加了你，现在我们可以开始聊天了"的自动加好友消息——这 **不是** 用户说的，是系统消息。此时直接回复 NO_REPLY，不做任何回应。等待用户发出真正的第一条消息后再开始对话。
...
**渠道判断（呈现前必做）：** 读 `plan-export` skill 的 `{plan-export:baseDir}/config.json`，获取 `urlExportChannels` 列表。再读 workspace 的 `channel-source.json` 判断用户渠道。**这两个读取和上面的 `forward-calc` 放进同一个 Round 1 并行批次**——不要先算再读、拆成多轮。

- **若用户渠道在 `urlExportChannels` 中（如 Slack、Twilio 等）：** 不发文本方案。保存 PLAN.md 并运行 `plan-export` 拿到 URL——**这两步必须在同一个 `exec` 里完成**（脚本要读 PLAN.md，并行批次顺序无保证，拆两轮则多付 10 秒+）：

  ```bash
  cat > {workspaceDir}/PLAN.md <<'PLAN_EOF'
  # ……完整方案内容（基于本轮 forward-calc 输出，按下方〔方案细节块〕的内容组织）……
  PLAN_EOF
  ...
  分隔符保持单引号（`<<'PLAN_EOF'`），内容不会被 shell 展开；内容里不要出现与分隔符相同的整行。同批次可并行加入 `health-profile.md` 的 BMR 编辑（不同文件，安全）。下一轮直接回复：URL 链接 + 必要的安全提醒（如 BMI 偏低警告）+ 确认问题。不要发用户信息摘要、不要发方案细节（热量目标、deficit、速度、完成时间等）——这些都在链接里。例如：
  > "你的减脂计划已生成！点击查看：[URL]。这个节奏合适吗？"

  **全程 3 轮封顶：** Round 1 = forward-calc ∥ config.json ∥ channel-source.json；Round 2 = 写 PLAN.md + 导出的单个 exec（∥ BMR 编辑）；Round 3 = 回复。
- **若用户渠道不在列表中：** 照常发送文本方案（见下方格式），不生成 URL。**2 轮封顶：** Round 1 = forward-calc（渠道信息已在 Round 1 一并读过）；Round 2 = 文本方案回复。
...
**轮次编排（≤2 轮）：** PLAN.md 写入和 BMR 编辑是不同文件，放进 Round 1 同一个并行批次；Round 2 直接回复过渡语。（URL 渠道在呈现时已保存并导出 PLAN.md，确认轮无需重写——只补 BMR 编辑。）
...
2. **激活 `notification-manager`** — 触发它运行 `batch-create-reminders.sh`，创建所有 cron 任务（餐前提醒、称重提醒、周报、每日复盘、饮食模式检测）。传 `--skip-existing`，重跑安全。此步静默——此刻绝不向用户提提醒或 cron。
   **轮次编排（本 turn ≤3 轮）：** Round 1 = 编辑 `health-profile.md`（Meal Schedule）；Round 2 = 运行 `batch-create-reminders.sh`（脚本要读刚写的 Meal Schedule，不能和编辑放同一并行批次）；Round 3 = 下面的确认 + Round 3 提问。中间不插任何额外的读取或思考轮。
...
与 Step 3 相同，先检查用户渠道是否在 `urlExportChannels` 中（已在 Step 3 读过 config.json 和 channel-source.json，可复用结果——不要重读）。

- **URL 渠道（3 轮封顶）：**
  - **Round 1（一个并行批次）：** `planner-calc.py macro-targets` ∥ 读 `{meal-planner:baseDir}/references/meal-plan-schema.md`（若 plan-export 文档不在上下文，也并入此批）。schema 定义 MEAL-PLAN.md 的**严格 Markdown 格式**（特别注意：metadata 用 `- Key: Value` 格式，Day 用 `## Day N | DayName | macros` 格式）。
  - **Round 2（一个并行批次）：** 一个 `exec` 内先 heredoc 写 MEAL-PLAN.md、再 `&&` 运行 `generate-and-send.sh` 拿 URL（命令模板同 Step 3 的 PLAN.md，`--input MEAL-PLAN.md --template meal-plan --key meal-plan`；脚本读 MEAL-PLAN.md，绝不和写入拆成并行兄弟调用或两轮）。同批并行加入「静默标记完成」的第二个 `exec`：`mark-onboarding-done.py && batch-create-reminders.sh --skip-existing`（两个脚本都操作 health-profile.md，必须在一个 exec 里用 `&&` 串联，不能再拆成并行调用）。MEAL-PLAN.md 内容直接在命令里一次性写出——不要先在 thinking 里起草。
  - **Round 3：** 回复用户。内容只包含 URL 链接。不要发宏量摘要、不要发饮食模板内容、不要发每日打卡流程介绍——这些都在链接里。例如：
  > "你的食谱已生成！点击查看：[URL]"
  不要在文本中包含任何饮食模板、宏量目标或打卡流程的内容。
- **非 URL 渠道：** 照常在文本中呈现完整饮食模板（见下方），不生成 URL。宏量呈现 turn ≤2 轮（Round 1 = `macro-targets` 计算；Round 2 = 宏量表回复 + 请确认）；用户确认后的模板 turn ≤2 轮（Round 1 = `message` tool 发送完整模板 + 打卡流程（见「长输出投递规则」）∥ 一个 `exec`：`mark-onboarding-done.py && batch-create-reminders.sh --skip-existing`；Round 2 = NO_REPLY 或一句简短确认）。
...
此脚本属于本 turn 的 Round 1 并行批次（和 schema/文档读取一起），不要单独占一轮。
...
**做法**：
1. 食谱模板 + 打卡流程组装成完整中文段
2. 用 `message(action=send, target=<user>, message=<完整内容>)` 主动发——并在 **同一个并行批次** 里加一个 `exec`：`mark-onboarding-done.py && batch-create-reminders.sh --skip-existing`（两个脚本都操作 health-profile.md，串在一个 exec 里跑，不要各占一轮）
3. 最终 final answer 留 NO_REPLY 或一句简短确认（不重复 message tool 已发的内容）
...
呈现饮食模板和每日打卡流程后（以下两步 **串成一个 `exec`**（`mark-onboarding-done.py && batch-create-reminders.sh --skip-existing`，两者都操作 health-profile.md），并入本 turn 已有的工具批次——URL 渠道并入写食谱 + 导出那一轮，非 URL 渠道并入 message 发送那一轮，不要各占一轮）：
...
**轮次编排（≤3 轮，见「轮次预算」）：** 本 turn 紧接着就是 Step 3 出方案——把两步合成一个 turn 编排：
- **Round 1（一个并行批次）：** `now.py` ∥ `weight-tracker.py save` ∥ 所有需要的文件读取 ∥ Step 3 的 `forward-calc` ∥ 渠道判断的两个读取（`{plan-export:baseDir}/config.json`、`channel-source.json`）。这些互不依赖，全部并行。
- **Round 2（一个并行批次）：** 写 `USER.md`、`health-profile.md`、`health-preferences.md`——三个不同文件，并行写入安全（URL 渠道再并行加入 Step 3 的「写 PLAN.md + 导出」单个 exec）。若画像文件内容已完全确定、无需 Round 1 的读取结果，这些写入直接并入 Round 1。
- **Round 3：** 呈现方案的回复（URL 或文本方案 + 确认问题）。

绝不要"读一轮 → 跑脚本一轮 → 写一轮 → 回复一轮"地拆成 4 轮。
